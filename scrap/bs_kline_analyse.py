...
def kline_application(df, N):
    """已知证券在起止时间内的每日 K 线类别：阳线，阴线。
         做如下统计：
        情景 1. 若证券股价连续 N 天下跌后出现大阳线，次日股价开盘上涨的次数（第 N+2 天）
        情景 2. 若证券股价连续 N 天上涨之后出现大阴线，次日股价开盘下跌的次数（第 N+2 天）
    :return:
    """
    ...
    print("若证券股价连续 N 天下跌后出现大阳线，次日股价开盘上涨的次数（第N+2 天）:" +
          str(total_counts_1_sub) + ",占比：" + str(total_counts_1_sub / total_counts_1))
    print("证券股价连续 N 天上涨之后出现大阴线的总次数：" + str(total_counts_2))
    print("若证券股价连续 N 天上涨之后出现大阴线，次日股价开盘下跌的次数（第N+2 天）:" +
          str(total_counts_2_sub) + ",占比：" + str(total_counts_2_sub / total_counts_2))
